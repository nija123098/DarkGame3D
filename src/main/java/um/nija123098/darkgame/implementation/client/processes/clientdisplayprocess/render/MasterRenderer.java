...
import um.nija123098.darkgame.implementation.client.processes.clientdisplayprocess.skybox.SkyboxRenderer;
...
public class MasterRenderer {
    ...
    private Map<TexturedModel, List<GraphicsEntity>> entites = new HashMap<TexturedModel, List<GraphicsEntity>>();
    ...
    public void render(Cam camera){
        List<Light> lights = new ArrayList<Light>();
        entites.forEach((texturedModel, graphicsEntities) -> graphicsEntities.forEach(graphicsEntity -> lights.addAll(graphicsEntity.getLights())));
        this.prepare();
        this.shader.start();
    ...
    public void processEntity(GraphicsEntity entity){
        ...
        List<GraphicsEntity> batch = entites.get(entityModel);
        if (batch != null){
            batch.add(entity);
        }else{
